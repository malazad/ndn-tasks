...
Consumer::GetTypeId(void)
{
  static TypeId tid =
    TypeId("ns3::ndn::Consumer")
	  ...
	  .AddAttribute("ConsumerName", "Name of the Consumer", StringValue("/"),
					MakeNameAccessor(&Consumer::m_consumerName), MakeNameChecker())
...
Consumer::SendPacket()
{
  ...
  if(m_currentFileNumber<=m_end)
  {
	  shared_ptr<Interest> interest = make_shared<Interest>();
	    shared_ptr<Name> interestNameSync = make_shared<Name>(m_interestName);
	    interestNameSync->append("sync");
	    interestNameSync->append(m_consumerName);

	    shared_ptr<Name> interestNameFile = make_shared<Name>("/file");
	    shared_ptr<Name> interestFinalName = make_shared<Name>();

	    if(m_isSynced == 0)
	    {
	  	  interestFinalName = interestNameSync;
	  	  m_isSynced++;

	    }

	    else if((m_currentFileNumber >= m_start) && (m_currentFileNumber <= m_end))
	  	  {
	  		  	  interestNameFile->appendSequenceNumber(m_currentFileNumber);
	  		  	  interestFinalName = interestNameFile;
	  		  	  //NS_LOG_INFO("Current File Number = "<< m_currentFileNumber);
	  		  	  m_currentFileNumber++;
	  	  }


	    NS_LOG_INFO("> Requested file is :" << *interestFinalName);
	    // shared_ptr<Interest> interest = make_shared<Interest> ();

	    interest->setNonce(m_rand->GetValue(0, std::numeric_limits<uint32_t>::max()));
	    interest->setName(*interestFinalName);
	    interest->setCanBePrefix(false);
	    time::milliseconds interestLifeTime(m_interestLifeTime.GetMilliSeconds());
	    interest->setInterestLifetime(interestLifeTime);

	    WillSendOutInterest(m_currentFileNumber-1);
...
Consumer::OnData(shared_ptr<const Data> data)
{
  ...
  if(dataName.getSubName(2,1).equals("/sync"))
  {
	  ...
	  m_start = std::strtoul(content2.substr(7,2).c_str(),nullptr,10);
	  m_end = std::strtoul(content2.substr(21,2).c_str(),nullptr,10);
